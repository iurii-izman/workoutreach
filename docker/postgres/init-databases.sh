...
psql --set=ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname postgres \
  --set=n8n_password="$n8n_password" --set=business_password="$business_password" --set=dashboard_password="$dashboard_password" <<'SQL'
...
SELECT format('CREATE ROLE workoutreach_dashboard LOGIN PASSWORD %L NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION', :'dashboard_password')
