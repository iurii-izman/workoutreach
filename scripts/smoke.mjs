...
try {
  ...
  run('docker', ['compose', 'exec', '-T', 'workoutreach-postgres', 'psql', '-U', 'workoutreach_admin', '-d', 'workoutreach_business', '-c', "SELECT version FROM workoutreach.schema_migrations WHERE version IN ('001_stage_0_1','002_owner_campaign_stage_1','003_stage_2_mock_outbox','004_local_stage_2_runtime','005_local_model_budget') ORDER BY version;"]);
  ...
  writeFileSync(resolve(root, 'artifacts/evidence/docker-smoke.json'), `${JSON.stringify({
    ...
    migrations: ['001_stage_0_1', '002_owner_campaign_stage_1', '003_stage_2_mock_outbox', '004_local_stage_2_runtime', '005_local_model_budget'],
