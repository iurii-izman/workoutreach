...
try {
  ...
  const restored = docker([
    ...
    '-c', "SELECT json_build_object('migration_count', count(*), 'has_stage2', bool_or(version = '003_stage_2_mock_outbox'), 'has_local_runtime', bool_or(version = '004_local_stage_2_runtime'), 'has_model_budget', bool_or(version = '005_local_model_budget'), 'has_smtp', bool_or(version = '006_guarded_smtp_delivery'), 'has_stage3_sendability', bool_or(version = '007_stage_3_template_sendability'), 'has_dashboard', bool_or(version = '008_local_operator_dashboard'), 'has_owner_capacity', bool_or(version = '009_owner_daily_capacity'), 'has_migration_integrity', bool_or(version = '010_immutable_migration_registry'), 'has_contact_resolution', bool_or(version = '011_pilot_contact_resolution'), 'has_universal_resilience', bool_or(version = '012_universal_first_resilience'), 'checksums_complete', bool_and(checksum_sha256 IS NOT NULL)) FROM workoutreach.schema_migrations;",
  ], { capture: true });
  if (!restored.includes('"has_stage2" : true') || !restored.includes('"has_local_runtime" : true') || !restored.includes('"has_model_budget" : true') || !restored.includes('"has_smtp" : true') || !restored.includes('"has_stage3_sendability" : true') || !restored.includes('"has_dashboard" : true') || !restored.includes('"has_owner_capacity" : true') || !restored.includes('"has_migration_integrity" : true') || !restored.includes('"has_contact_resolution" : true') || !restored.includes('"checksums_complete" : true')) throw new Error('Restored database is missing required migration integrity evidence');
