...
- published date либо подтверждена, либо равна `null`;
- offer overlap использует только утверждения из versioned offer profile;
- персональная фраза содержит от 18 до 35 слов, если шаблон не задаёт более узкий диапазон;
...
Они не блокируют bootstrap, но обязательны до соответствующего этапа:

- текущая точная инструкция ChatGPT, используемая для анализа;
- финальная тема и шаблон письма с `{{PERSONALIZATION_PHRASE}}`;
- утверждённое описание продукта/предложения и запрещённые claims;
- язык первого шаблона;
- sender name, reply-to и корпоративный домен;
- выбранный mailbox provider;
- Telegram test/prod bot tokens и allowlisted IDs через secret channel;
...
Если материал ещё не предоставлен, реализация использует явно неотправляемые fixtures/placeholders и не придумывает production-текст за владельца.
