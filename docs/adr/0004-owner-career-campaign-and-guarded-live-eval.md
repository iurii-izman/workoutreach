...
3. The exact owner email is versioned as `email-ru-career-v1`. Only `COMPANY_NAME` and `PERSONALIZATION_PHRASE` are dynamic. The opt-out, signature, links and all other copy are static. The manifest remains `sendable=false`.
