...
ALTER TABLE approval_tokens ADD CONSTRAINT approval_tokens_action_check
  CHECK (action IN ('mock_send','smtp_send','regenerate','reject','select_recipient'));
...
ALTER TABLE operator_actions ADD CONSTRAINT operator_actions_action_check
  CHECK (action IN ('mock_send','smtp_send','regenerate','reject','select_recipient','cancel','refresh'));

ALTER TABLE outbox DROP CONSTRAINT IF EXISTS outbox_transport_check;
ALTER TABLE outbox DROP CONSTRAINT IF EXISTS outbox_status_check;
ALTER TABLE outbox DROP CONSTRAINT IF EXISTS outbox_provider_message_id_check;
ALTER TABLE outbox DROP CONSTRAINT IF EXISTS outbox_check;
ALTER TABLE outbox DROP CONSTRAINT IF EXISTS outbox_acceptance_shape_check;
ALTER TABLE outbox ADD CONSTRAINT outbox_transport_check CHECK (transport IN ('mock','smtp'));
ALTER TABLE outbox ADD CONSTRAINT outbox_status_check CHECK (status IN (
  'MOCK_PENDING','MOCK_CLAIMED','MOCK_ACCEPTED',
  'SMTP_PENDING','SMTP_CLAIMED','SMTP_ACCEPTED','SMTP_FAILED',
  'CANCELLED','SUPPRESSION_BLOCKED'
));
ALTER TABLE outbox ADD CONSTRAINT outbox_provider_message_id_check
  CHECK (provider_message_id IS NULL OR char_length(provider_message_id) BETWEEN 1 AND 500);
ALTER TABLE outbox ADD CONSTRAINT outbox_acceptance_shape_check CHECK (
  (transport='mock' AND ((status='MOCK_ACCEPTED')=(mock_accepted_at IS NOT NULL)) AND provider_message_id IS NULL)
  OR (transport='smtp' AND mock_accepted_at IS NULL AND (status<>'SMTP_ACCEPTED' OR provider_message_id IS NOT NULL))
);

CREATE INDEX IF NOT EXISTS outbox_smtp_claim_idx ON outbox(available_at,id)
  WHERE transport='smtp' AND status='SMTP_PENDING';

CREATE TABLE IF NOT EXISTS mail_runtime_controls (
  singleton boolean PRIMARY KEY DEFAULT true CHECK (singleton),
  live_send_enabled boolean NOT NULL DEFAULT false,
  mail_transport text NOT NULL DEFAULT 'disabled' CHECK (mail_transport IN ('disabled','smtp')),
  daily_send_limit integer NOT NULL DEFAULT 0 CHECK (daily_send_limit BETWEEN 0 AND 5),
  ...
  CHECK (
    ...
    OR (live_send_enabled=true AND mail_transport='smtp' AND daily_send_limit BETWEEN 1 AND 5 AND kill_switch_enabled=false)
  )
);

INSERT INTO mail_runtime_controls(singleton) VALUES(true) ON CONFLICT DO NOTHING;
...
BEGIN
  IF p_enabled AND p_daily_limit NOT BETWEEN 1 AND 5 THEN
...
BEGIN
  ...
  SELECT * INTO v_outbox FROM outbox WHERE transport='smtp' AND status='SMTP_PENDING' AND available_at<=CURRENT_TIMESTAMP ORDER BY available_at,id FOR UPDATE SKIP LOCKED LIMIT 1;
  ...
  RETURN QUERY SELECT o.id,o.job_id,o.draft_version,d.recipient_email,d.subject,d.body_text,d.body_html,d.attachment_filename,d.attachment_sha256,j.telegram_chat_id FROM outbox o JOIN drafts d ON (d.job_id,d.draft_version)=(o.job_id,o.draft_version) JOIN jobs j ON j.job_id=o.job_id WHERE o.id=v_outbox.id;
END;
$$;

CREATE OR REPLACE FUNCTION complete_smtp_outbox(p_outbox_id bigint,p_worker_id text,p_provider_message_id text)
RETURNS boolean LANGUAGE plpgsql SECURITY INVOKER SET search_path=workoutreach,public AS $$
DECLARE v_job_id text;
BEGIN
  UPDATE outbox SET status='SMTP_ACCEPTED',provider_message_id=p_provider_message_id,updated_at=CURRENT_TIMESTAMP WHERE id=p_outbox_id AND transport='smtp' AND status='SMTP_CLAIMED' AND claimed_by=p_worker_id RETURNING job_id INTO v_job_id;
  IF NOT FOUND THEN RETURN false; END IF;
  UPDATE jobs SET status='PROVIDER_ACCEPTED' WHERE job_id=v_job_id;
  INSERT INTO audit_log(job_id,event_type,actor_type,safe_metadata) VALUES(v_job_id,'SMTP_PROVIDER_ACCEPTED','system',jsonb_build_object('outbox_id',p_outbox_id));
  RETURN true;
END;
$$;

CREATE OR REPLACE FUNCTION fail_smtp_outbox(p_outbox_id bigint,p_worker_id text,p_safe_code text)
RETURNS boolean LANGUAGE plpgsql SECURITY INVOKER SET search_path=workoutreach,public AS $$
DECLARE v_job_id text;
BEGIN
  UPDATE outbox SET status='SMTP_FAILED',updated_at=CURRENT_TIMESTAMP WHERE id=p_outbox_id AND transport='smtp' AND status='SMTP_CLAIMED' AND claimed_by=p_worker_id RETURNING job_id INTO v_job_id;
  IF NOT FOUND THEN RETURN false; END IF;
  UPDATE jobs SET status='FAILED',error_code=left(p_safe_code,120) WHERE job_id=v_job_id;
  INSERT INTO audit_log(job_id,event_type,actor_type,safe_metadata) VALUES(v_job_id,'SMTP_FAILED','system',jsonb_build_object('outbox_id',p_outbox_id,'safe_code',left(p_safe_code,120)));
  RETURN true;
END;
$$;

INSERT INTO schema_migrations(version) VALUES('006_guarded_smtp_delivery') ON CONFLICT DO NOTHING;
