...
> Статус: Final v1.6 — fixed universal active runtime
...
- отдельный hardened Node.js service — текущий critical path: crawl, deterministic contact gates, fixed universal template, Telegram и SMTP dispatcher;
- PostgreSQL — единственный источник бизнес-состояния, очереди и идемпотентности;
- OpenAI Responses API — замороженный неактивный eval-модуль; active runtime не вызывает модель и не получает API key;
...
Оператор отправляет боту только URL публичного сайта компании. Система безопасно изучает ограниченный набор страниц, находит опубликованные контактные адреса и формирует точный owner-approved универсальный черновик v3 без динамических placeholders и model calls.
...
- фиксированный универсальный шаблон без AI-анализа;
- строгая проверка источника опубликованного контакта;
...
6. После contact/source checks система загружает byte-stable owner-approved шаблон v3 и не вызывает модель.
7. Шаблонизатор подтверждает отсутствие динамических placeholders и вычисляет hash неизменяемого subject/text/html bundle.
8. Бот присылает полный предпросмотр письма, адресат, источник контакта и предупреждения.
9. Оператор выбирает «Отправить» или «Отклонить»; перегенерация фиксированного текста отсутствует.
10. После «Отправить» система атомарно проверяет suppression и дедупликацию, создаёт outbox-запись и выполняет одну отправку.
11. Бот обновляет статус по фактически полученным событиям.
...
Предпросмотр действителен 24 часа. После истечения срока требуется повторная проверка URL/контакта. Фиксированный universal-only draft не имеет действия перегенерации.
...
flowchart LR
    ...
    CR --> X
    X --> T["Fixed universal template v3 · zero placeholders"]
    T --> TG
...
- active runtime не получает OpenAI key и не вызывает модель;
- замороженный eval-модуль не имеет почтовых, Telegram- или БД-инструментов;
...
- загрузка owner-approved fixed universal template v3;
- проверка нулевого placeholder contract и template hash;
- ноль OpenAI-вызовов и company-specific claims;
...
## 10. Замороженный OpenAI eval-модуль

### 10.1. API и модель

- эти требования применяются только при отдельном будущем размораживании eval-модуля; active runtime их не выполняет;
...
- исторический eval-default с 28 июля 2026 года: `gpt-5.6-luna` для обеих строго ограниченных model roles; active runtime model не выбирает;
...
Замороженный eval-код по-прежнему понимает исторические режимы `optional` и `required`, однако active Docker runtime жёстко задаёт `OUTREACH_PERSONALIZATION_MODE=off`, не монтирует OpenAI key и не позволяет включить персонализацию через `.env`.

Исторический контракт `OUTREACH_PERSONALIZATION_MODE`:

- `off` — только универсальный текст, ноль model calls;
- `optional` — только неактивный eval-режим: персонализация улучшается при успехе, но не блокирует универсальный черновик v2;
- `required` — диагностический/eval-режим со старым fail-closed поведением.

При `PHRASE_SENTENCE_COUNT` разрешён только семантически нейтральный локальный ремонт внешних кавычек или отсутствующего конечного знака. Реальное нарушение длины, числа предложений, согласования, перспективы участников или конкретности связи получает один повтор только вызова B с безопасным error code без предыдущего сырого model output. Повторный провал создаёт `UNIVERSAL_FALLBACK`, а не `FAILED`.

Персональный абзац состоит ровно из двух коротких фраз и обязан явно разделять субъектов. Первая фраза передаёт опубликованный факт и действия компании без первого лица и является законченным предложением со сказуемым, а не заголовком «Ваш кейс внедрения…». Вторая начинается строго с «Мне близки такие проекты/задачи, где…» и на уровне результата объясняет человеческую связь; это единственный маркер первого лица. Клиентский кейс нельзя превращать во внутреннее внедрение адресата через «у вас внедрён». Нельзя приписывать компании «ваш аналитический цикл», опыт, профиль, компетенции или навыки кандидата. Абстрактные выводы через формы слова «релевантный», перегруженные конструкции с `;`/`:`, повтор стажа, числа внедрений и других сведений из фиксированной части письма не принимаются: связь должна быть конкретной, лёгкой и естественной.
...
## 12. Замороженные evidence gate и правила персональной фразы
...
- персональный абзац содержит предпочтительно 25–32 слова и строго не выходит за диапазон 18–35 слов для утверждённой карьерной кампании;
...
Эти правила сохраняются для исторических evals. В active runtime персональная фраза отсутствует. Провал URL, contact, suppression, attachment, template или transport safety gate остаётся fail-closed.

## 13. Шаблон письма

Active runtime не использует AI. Тема, основной текст и подпись хранятся как единый утверждённый versioned template v3. В индивидуальном пилоте отдельная opt-out-фраза в теле письма не используется; любой отрицательный ответ вручную вносится в suppression до следующего контакта.

Активные файлы:

```text
templates/email/ru/v3/subject.txt
templates/email/ru/v3/body.txt
templates/email/ru/v3/body.html
templates/email/ru/v3/manifest.json
product/offer-profile.v1.yaml
```

Активный v3 не допускает placeholders. Subject, plain text и HTML являются byte-stable owner-approved содержимым; URL, company name, email или данные сайта в письмо не подставляются.
...
- финальная русская тема и owner-approved fixed universal шаблон v3 без placeholders;
- утверждённый профиль кандидата, запрещённые claims и актуальное CV как внешний read-only asset;
- карьерная цель и партнёры Bitrix24 в разных странах; русский fixed template применяется только там, где язык уместен для адресата;
- Telegram test-bot token через ignored `.env`; существующий OpenAI API key не используется active runtime;
